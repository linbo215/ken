...
def main():
        ...
        if current_line.startswith('#EXTINF') and i+1 < len(lines):
            ...
            if url.startswith('http'):
                try:
                    ...
                    # --- 🛠️ 核心修复：更健壮的频道名安全提取逻辑 ---
                    name = "Unknown"
                    if ',' in current_line:
                        # 使用 split 拆分，安全拿取最后一个元素
                        parts = current_line.split(',', 1)
                        if len(parts) > 1 and parts[1].strip():
                            name = parts[1].strip()
                        else:
                            # 降级防御：如果逗号后面是空的，尝试从 tvg-name 里捞一下名字
                    ...
                    # 单行异常仅作跳过处理，绝不拖垮整个主流程
                    print(f"⚠️ 解析第 {i+1} 行时遇到异常已跳过: {parse_err}")
                    continue
    ...
    if not groups:
        print("⚠️ 未能在 M3U 文件中解析到任何有效的直播源节点，请检查文件内容。")
        return

    # 抽样
    tasks = []
    for ip_port, urls in groups.items():
        # 如果 urls 为空，random.sample 会报错，这里做一层前置保护
